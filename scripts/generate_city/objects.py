...
def setAttributeForObjectsByName(object_name, attribute_path, values):
    normalized_values = (
        [values] if not isinstance(values, (list, tuple, set)) else list(values)
    )
    objects = ce.getObjectsFrom(ce.scene, ce.withName(object_name))
    for o in objects:
...
def setStreetVisibility(visible: bool):
    l = ce.getObjectsFrom(ce.scene, ce.isLayer, ce.withName("'streets'"))[0]
